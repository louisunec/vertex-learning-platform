...
- Approved content is read-only in the Studio, and a draft that changes the content of any published version (approved or archived) cannot publish. The API does not enforce this, so attempts record the exact `_id` and `_rev` they were delivered. See "Authorization boundaries" in §16: these Studio rules are workflow controls, not API authorization.
...
- **Prerequisite edges** (`conceptPrerequisite`).
  ...
  - Edges have no delete, unpublish, duplicate or schedule-publish action in the Studio: an edge is retired, not removed, and rejected edges are kept for audit.
...
- **Studio gates are editorial workflow controls, not API authorization.** The publish gates, the hidden and disabled document actions, and read-only fields all run client-side in the Studio. They guide editors; they do not stop a request that reaches the Content Lake API another way.
- **Learner reads are the enforcement point.** A learner-facing read must require approved, eligible, published records, never "anything the token can see":
  - Assessments: `LESSON_PRACTICE_ITEMS_QUERY` (`sanity/queries/assessments.ts`) reads the published perspective. It requires `reviewStatus == "approved"` and `sourceStatus == "current"`, excludes `drafts.` and `versions.` ids, and keeps the latest servable version per family. `toLearnerAssessments` then drops any row outside its strict learner schema. `lib/assessments/learner-query.test.ts` covers this.
  - Concepts and edges: PR-3 has no learner read. Any future reader must read the published perspective, use only `approved` concepts with `sourceStatus == "current"` and `approved` edges between them, resolve ids through `lib/concepts/resolve.ts` (anything not approved is `unavailable`), and read only a graph that `validate:concepts` accepts.
- **Separate boundaries the gates do not cover:**
  - **Privileged API writers.** Any write-capable token or CLI session can create, publish or delete documents directly, gates or not. Keep such credentials server-side and offline (generators, imports), give the learner app a read-only (Viewer) token, and treat every write-capable credential as editor-equivalent.
  - **Releases.** Content added to a Release is published with the Release, outside the document publish action, so the gates do not apply there.
