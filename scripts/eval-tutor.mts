 ...
 *
 * `--runs` repeats the cases and reports latency by stage. `--packet`
 * writes the human review packet from the last run (`scripts/tutor-packet.mts`:
 * short excerpts; the full-text copy goes to the gitignored `docs/evals/local/`). The `tutor-terms-v1`
...
if (packetFile) {
  // Short excerpts in the committed packet; the full text only in the gitignored local copy.
  ...
  const section = {
    ...
    note: `Prompts \`${TUTOR_PROMPT_VERSION}\` and \`${TUTOR_SUPPORT_PROMPT_VERSION}\`, \`${TUTOR_MODEL_ID}\`, published Sanity dataset (read-only).`,
    results: last.map((result) => ({caseId: result.caseId, scope: result.scope, error: result.error, answer: result.answer, evidenceCount: result.evidence.length})),
  }
