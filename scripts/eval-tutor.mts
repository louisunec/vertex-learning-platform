 ...
 * writes the human review packet from the last run. The `tutor-terms-v1`
...
function reviewPacket(last: Result[]): string {
  const lines = [
    ...
    '',
    `Generated by \`npm run eval:tutor -- --packet …\` on ${new Date().toISOString().slice(0, 16)}Z against the published Sanity dataset (read-only) and OpenAI \`${TUTOR_MODEL_ID}\`; prompts \`${TUTOR_PROMPT_VERSION}\` and \`${TUTOR_SUPPORT_PROMPT_VERSION}\`.`,
    '',
