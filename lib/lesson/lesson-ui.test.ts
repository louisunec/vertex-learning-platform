...
import {citationText, groupCitations} from './citations.ts'
...
describe('decideLessonFeatures', () => {
  ...
  it('turns everything on only with every flag, a YouTube video, and reviewed items', () => {
    assert.deepEqual(decide({}), {check: true, hints: true, tutor: true})
  })
  ...
  it('hides everything when lesson-integration or learner-evidence is off', () => {
    for (const off of [{lessonIntegration: false}, {learnerEvidence: false}]) {
      assert.deepEqual(decide(off), {check: false, hints: false, tutor: false})
    }
  })

  it('degrades one prerequisite at a time', () => {
    assert.deepEqual(decide({helpPolicy: false}), {check: true, hints: false, tutor: false})
    assert.deepEqual(decide({tutor: false}), {check: true, hints: true, tutor: false})
    assert.deepEqual(decide({}, 'vimeo'), {check: true, hints: true, tutor: false})
    assert.deepEqual(decide({}, 'youtube', 0), {check: false, hints: false, tutor: true})
  })
})
