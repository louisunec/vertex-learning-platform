...
const MESSAGES: Record<LearnerErrorCode, string> = {
  ...
  explanation_in_progress: 'This explanation is already being read; retry shortly',
...
const STATUS: Record<LearnerErrorCode, number> = {
  ...
  rate_limited: 429,
  explanation_in_progress: 409,
  unavailable: 503,
  internal_error: 500,
}

export function learnerError(code: LearnerErrorCode): Response {
  return learnerJson({error: MESSAGES[code], code, retryable: code === 'unavailable' || code === 'rate_limited' || code === 'explanation_in_progress'}, STATUS[code])
}
